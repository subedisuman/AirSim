...
void MultirotorPawnSimApi::initialize()
{
    ...
    if(dekf_shared_res_ == nullptr) std::cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< <<<<<< <<< << dekf_shared_res_ null in MRPS API initialize()" << std::endl;
    ...
    if (getVehicleSetting()->vehicle_type == "" || //default config
        ...
        if (is_header_line) {
            return common_line +
                ...
                "POD_LP_X\tPOD_LP_Y\tPOD_LP_Z\tPOD_LP_VAR_X\tPOD_LP_VAR_Y\tPOD_LP_VAR_Z\tPRED_ENT\t" +
                "QUAT_NORM\t" +
        ...
        ss << ekf_meas.lp_center.x() << "\t" << ekf_meas.lp_center.y() << "\t" << ekf_meas.lp_center.z() << "\t";
        ss << ekf_meas.lp_center_var.x() << "\t" << ekf_meas.lp_center_var.y() << "\t" << ekf_meas.lp_center_var.z() << "\t" << ekf_meas.predictive_entropy << "\t";
